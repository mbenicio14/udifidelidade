        ...
        <div class="grid lg:grid-cols-2 lg:h-full">
            <div class="flex justify-center items-center py-6 px-4 lg:py-6 sm:px-0">
                <div class="w-full space-y-4 md:space-y-6 max-w-lg xl:max-w-lg p-6">
                    ...
                    <div class="flex p-4 mb-4 text-sm text-blue-800 border-2 border-blue-300 rounded-lg bg-blue-50 dark:bg-gray-800 dark:text-blue-400 dark:border-blue-800 cursor-pointer" role="alert" onclick="document.getElementById('email').value='admin@example.com';document.getElementById('password').value='{{ env('APP_DEMO_PASSWORD', 'welcome123') }}';">
                        <x-ui.icon icon="info" class="flex-shrink-0 w-5 h-5" />
                        <div class="ml-3 text-sm font-medium">
                            To access the demo, use the following login credentials: <span class="font-extrabold">admin@example.com / {{ env('APP_DEMO_PASSWORD', 'welcome123') }}</span> (click to autofill).
                        </div>
                      </div>
                    @endif
